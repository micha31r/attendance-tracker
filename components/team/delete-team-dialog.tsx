...
import { deleteTeam } from "@/lib/data/team"
import { useRef, useState } from "react"

export function DeleteTeamDialog({ name, id, trigger }: { name: string, id: string, trigger: React.ReactNode }) {
  ...
  async function onSubmit() {
    ...
    if (inputRef.current.value !== name) {
      alert("Team name does not match. Please try again.")
      setDisabled(false)
      return
    }
    ...
    <Dialog>
      <div>
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div>
            <p className="text-sm mb-1">Enter the team name to confirm deletion:</p>
            <p className="text-md text-primary pointer-events-none select-none">{name}</p>
          </div>
          <div className="grid gap-4">
            <div className="grid gap-3">
              <Label htmlFor="name-1">Team name</Label>
              <Input ref={inputRef} id="name-1" name="name" autoComplete="off" placeholder="Team name" />
            </div>
          </div>
          <DialogFooter>
            <DialogClose asChild>
